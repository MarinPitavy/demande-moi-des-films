...
class MovieLens:
    ...
    def load_ratings():
        ratings = []
        with open('./ml-latest-small/ratings.csv', 'rt', encoding ='utf8') as ratingsFile:
    ...
    def load_simplified_ratings():
        ratings = []
        with open('./ml-latest-small/ratings-popular-simplified.csv', 'rt', encoding ='utf8') as ratingsFile:
            raw_ratings = csv.reader(ratingsFile, delimiter=',')
            is_first = True
            for rating in raw_ratings:
                if is_first:
